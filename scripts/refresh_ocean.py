...
Reads the 1/12° ARCO store and coarsens ×3 down to 1/4° (atmosphere-grid parity).
...
STRIDE = 3   # 1/12° → 1/4° (2040 % 3 == 0, so the poles-to--80° span lands exactly)
WINDOW = 7   # NaN-fill window in full-res cells: ±3/12° = one full output cell


def coarsen(full):
    """
    1/12° → 1/4°: sample every STRIDE-th point, but where the sampled point is land
    (NaN), fall back to the mean of the surrounding WINDOW×WINDOW full-res window. A
    plain stride marks an output cell "land" whenever its exact sample point is, giving
    the sea a land mask a whole cell fatter than the vector coastline — charcoal
    staircase blocks jutting into the sea (user bug report). The window fill pulls the
    data's coast past every output cell that touches real ocean, and the finer 1/4°
    cells shrink what little staircase remains (second user pass: 1/3° + 5×5 still left
    single-cell nubs on convoluted coasts).
    """
    sampled = full[::STRIDE, ::STRIDE]
    half = WINDOW // 2
    pad = np.pad(full, half, constant_values=np.nan)
    total = np.zeros(sampled.shape)
    count = np.zeros(sampled.shape)
    for dy in range(WINDOW):
        for dx in range(WINDOW):
            shifted = pad[dy:dy + full.shape[0]:STRIDE, dx:dx + full.shape[1]:STRIDE]
...
def fetch(product, day):
    """Latest daily mean ≤ day from the credentialed 1/12° store, coarsened to 1/4°."""
    ...
    return (fields, sel.latitude.values[::STRIDE], sel.longitude.values[::STRIDE],
            when, float(sel.depth.values))
